...
config:
	...
	@echo "Add bda-mota.42.fr in /etc/hosts..."
		@if ! grep -q "bda-mota.42.fr" /etc/hosts; then \
		echo "127.0.0.1 $(USER).42.fr" | sudo tee -a /etc/hosts > /dev/null; \
		else echo "bda-mota.42.fr already exists in /etc/hosts!"; \
	fi
...
fclean: clean
	...
	rm ./srcs/.env
	sudo sed -i '/bda-mota\.42\.fr/d' /etc/hosts
	@echo "Removing the data directories..."
	@if [ -d "/home/bda-mota/data" ]; then \
		sudo rm -rf /home/bda-mota/data; \
	fi
